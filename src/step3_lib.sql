...
CREATE or replace FUNCTION tlib.n2c_check_normalize(p_term text, p_ns int) RETURNS int AS $wrap$
  select coalesce(fk_canonic,id) from tlib.n2c_tab(tlib.normalizeterm(p_term),p_ns)
$wrap$ language SQL immutable;
